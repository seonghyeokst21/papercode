...
from dataset_path import CreateDatasetSynthesis
...
def sample_and_test(args):
    ...
    save_dir = exp_path + "/generated_samples/epoch_{}_ct2t1_for_multi_figure".format(epoch_chosen)
    ...
    for iteration, (data, paths) in enumerate(data_loader):
        ...
        if not os.path.exists(patient_save_dir_x+filename_y[:8]):
            os.makedirs(patient_save_dir_x+filename_y[:8])
        if not os.path.exists(patient_save_dir_y+filename_y[:8]):
            os.makedirs(patient_save_dir_y+filename_y[:8])
        # if filename_y[:8] == '02225372':
        #     continue
        ...
        # if '26' not in filename_y[9:-4]:
        #     continue
        print(str(iteration))
        print(filename_y[9:-4])
        print(loss2[0, iteration], ssim_loss2[0, iteration])
        

        # torchvision.utils.save_image(fake_sample2, os.path.join(patient_save_dir_y+filename_y[:8], f"{filename_y[2:-4]}.png"[1:]), normalize=True)
        # fake_sample2 = torch.cat((source_data, fake_sample2, real_data),axis=-1)
        torchvision.utils.save_image(fake_sample2, os.path.join(patient_save_dir_y+filename_y[:8], f"{filename_y[9:-4]}.png"[:]), normalize=True)
...
# python3 test.py --image_size 256 --ch_mult 1 1 2 2 4 4 --contrast1 CT --contrast2 T2 --input_path ../datasets/new_brain/test --output_path ./output_brain/for/results_minmax0525_cross  --which_epoch 475
