...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    if conditional:
      ...
      modules[-1].weight.data = default_initializer()(modules[-1].weight.shape) #uniform 으로 weight 초기화
      nn.init.zeros_(modules[-1].bias) # 바이어스 0으로 초기화 
    ...
    # if resblock_type == 'scoredf':
    #   ResnetBlock = functools.partial(ResnetBlockScoreDF,
    #                                  act=act,
    #                                  dropout=dropout,
    #                                  init_scale = init_scale , 
    #                                  skip_rescale = skip_rescale,
    ...
    channels = config.num_channels # default = 3 
    if progressive_input != 'none': # residual 
      input_pyramid_ch = channels # 

    modules.append(conv3x3(channels, nf)) #(3, 128)
    hs_c = [nf]

    in_ch = nf
    for i_level in range(num_resolutions): # len(ch_mult)
      # Residual blocks for this resolution
      for i_block in range(num_res_blocks): # default = 2
        out_ch = nf * ch_mult[i_level]
        modules.append(ResnetBlock(in_ch=in_ch, out_ch=out_ch)) # (128, 128 * a) -> (128*a,128 * a)
        in_ch = out_ch

        if all_resolutions[i_level] in attn_resolutions: #(16,)
